...
class wideResnet22Basic(nn.HybridBlock):
    def __init__(self, num_classes):
        super(wideResnet22Basic, self).__init__()
        with self.name_scope():
            ...
            # Stage 1 (3 total)
            net.add(BasicBlock(16*6, stride=1, dim_match=False))
            for _ in range(2):
                net.add(BasicBlock(16*6, stride=1, dim_match=True))
            # Stage 2 (3 total)
            net.add(BasicBlock(32*6, stride=2, dim_match=False))
            for _ in range(2):
                net.add(BasicBlock(32*6, stride=1, dim_match=True))
            # Stage 3 (3 total)
            net.add(BasicBlock(64*6, stride=2, dim_match=False))
            for _ in range(2):
                net.add(BasicBlock(64*6, stride=1, dim_match=True))
